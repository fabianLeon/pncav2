...
switch ($task) {
    ...
    case 'encuesta':
        ...
        foreach ($secciones as $s) {
            ...
            echo '<div class="panel panel-primary" ><div class="panel-heading">' . $html->traducirTildes($s['numero'] . " " . $s['nombre']) . "</div>";
            //-----------------preguntas base----------------------------------
            ...
            foreach ($preguntas_base as $pb) {
                ...
                switch ($pb['tipo']) {
                    ...
                    case 4:
                        $value[$pb['id']] = $ejeData->getRespuestaPreguntaDeEncuesta($pb['id'], $id_add);
                        $saltos = $ejeData->arregloSaltarAPregunta($pb['id']);
                        $form->crearTextArea($pb['id'], $value[$pb['id']], 30, 0, 'onChange="saltar_pregunta_onChange(\'' . ($pb['id']) . '\',\'' . $saltos . '\',\'' . $pb['tipo'] . '\');"');
                        $form->crearDivError('error_pregunta_' . $pb['id'], ERROR_EJECUCION_CC);
                        break;
                    case 5:
                        $value[$pb['id']] = $ejeData->getRespuestaPreguntaDeEncuesta($pb['id'], $id_add);
